...
type LoopNode =
	| Rule.Node & { type: "ForStatement" }
	| Rule.Node & { type: "ForOfStatement" }
	| Rule.Node & { type: "ForInStatement" }
	| Rule.Node & { type: "WhileStatement" }
	| Rule.Node & { type: "DoWhileStatement" };

const noQuadraticLoopOperationsRule: Rule.RuleModule = {
	meta: {
		docs: {
			description:
				"Detects O(n) operations inside loops that cause O(n²) complexity",
	...
	create(context: Rule.RuleContext): Rule.RuleListener {
		...
		const outerScopeVariables = new Map<string, Rule.Node>();

		// Helper: Check if a node is inside a loop
		function getEnclosingLoop(node: Rule.Node): LoopNode | null {
			...
			while (current) {
				...
				if (
					...
					if (
						...
						return { type: "ForOfStatement" } as LoopNode;
					}
				}
		...
		function isDeclaredInsideLoop(
			...
			let current: Rule.Node | undefined = declaration;
			while (current) {
				if (current === loopNode) {
					return true;
				}
			...
			let checkNode: Rule.Node | undefined = currentNode;
			while (checkNode) {
				...
				if (
					...
					if (
						...
						if (
							...
							let declCurrent: Rule.Node | undefined = declaration;
							while (declCurrent) {
		...
		function isUsedInLoopCondition(
			varName: string,
			loopNode: LoopNode
		): boolean {
				...
				if (
					...
					if (methodName === "unshift") {
						...
						context.report({
							node,
							message:
								"unshift() in loop causes O(n²) complexity. Consider: use push() then reverse once, iterate in reverse order, or use a deque data structure",
						});
						return;
					}
					...
					if (methodName === "shift") {
						...
						if (isUsedInLoopCondition(varName, loopNode)) {
							context.report({
								node,
								message:
									"shift() in loop causes O(n²) complexity. Consider: use an index pointer, iterate in reverse with pop(), or use a deque data structure",
							});
							return;
						}
